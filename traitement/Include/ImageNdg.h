...
public:
	...
	_declspec(dllexport) CImageNdg& choixPalette(const std::vector<int>& map) { // couleurs donn�es par tableau de 256 couleurs RGB)
		for (int indice = 0; indice < 256; indice++) {
	...
	// seuillage
	_declspec(dllexport) CImageNdg seuillage(const std::string& methode = "auto", int seuilBas = 128, int seuilHaut = 255); // seuillaue automatique Otsu par d�faut, sinon "manuel" et choix des seuils Bas et Haut
